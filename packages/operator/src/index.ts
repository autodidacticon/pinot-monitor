...
import { getAuditLog, logAudit } from './audit.js';
import { persistAuditEntry } from './audit-persistence.js';
import { canAttempt, recordAttempt } from './circuit-breaker.js';
import { config } from './config.js';
import { getNovelIncidents, recordNovelIncident } from './novel-incidents.js';
import { matchRunbook } from './runbooks/definitions.js';

// Metrics
const metrics = new MetricsRegistry();
const incidentsReceived = metrics.counter(
  'operator_incidents_received_total',
  'Total incidents received'
);
const incidentsDispatched = metrics.counter(
  'operator_incidents_dispatched_total',
  'Incidents dispatched to mitigator'
);
const incidentsNoRunbook = metrics.counter(
  'operator_incidents_no_runbook_total',
  'Incidents with no matching runbook'
);
const circuitBreakerTrips = metrics.counter(
  'operator_circuit_breaker_trips_total',
  'Circuit breaker activations'
);
const triageLatency = metrics.histogram(
  'operator_triage_duration_seconds',
  'Triage processing time'
);
const rateLimitRejections = metrics.counter(
  'operator_rate_limit_rejections_total',
...
const activeRemediations = new Map<
  string,
  { correlationId: string; runbookId: string; startedAt: string }
>();
...
async function sendAlert(incident: Incident, reason: string): Promise<void> {
  console.log(
    `[ALERT] ${reason} — ${incident.severity} on ${incident.component}: ${incident.evidence.join('; ')}`
  );
...
async function verifyIncidentState(
  incident: Incident
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        confirmed: true,
        reason: `Monitor returned HTTP ${res.status}, proceeding with dispatch`,
      };
    }
    const text = await res.text();
    ...
    if (jsonMatch) {
      try {
        ...
        return {
          confirmed: true,
          reason: 'Could not parse monitor response, proceeding with dispatch',
        };
      }
    }
    ...
    return {
      confirmed: true,
      reason: 'Monitor response not in expected format, proceeding with dispatch',
    };
  } catch (err: unknown) {
    clearTimeout(timeout);
...
async function triageIncident(incident: Incident): Promise<TriageResult> {
  ...
  if (!canAttempt(runbook.id, incident.component, runbook.maxRetries, runbook.cooldownMs)) {
    ...
    if (runbook.escalateAfterRetries) {
      await sendAlert(
        incident,
        `Circuit breaker open: ${runbook.id} exhausted ${runbook.maxRetries} retries`
      );
    }
    const entry = {
      timestamp: new Date().toISOString(),
    ...
    return {
      ...
      message: `Circuit breaker open for ${runbook.id}`,
    };
  }
  ...
  if (config.trustLevel < runbook.minTrustLevel) {
    ...
    if (config.trustLevel === 0) {
      ...
      return {
        action: 'logged',
        runbookId: runbook.id,
        message: `Observe mode (trust ${config.trustLevel} < ${runbook.minTrustLevel}): ${proposedAction}`,
      };
    }
    ...
    await sendAlert(
      incident,
      `Suggested action (trust ${config.trustLevel} < ${runbook.minTrustLevel}): ${proposedAction}`
    );
    logAudit({
      timestamp: new Date().toISOString(),
      agent: 'operator',
    ...
    return {
      action: 'suggested',
      runbookId: runbook.id,
      message: `Suggested (trust ${config.trustLevel} < ${runbook.minTrustLevel}): ${proposedAction}`,
    };
  }
  ...
  if (config.trustLevel === 2 && incident.severity === 'CRITICAL') {
    ...
    return {
      action: 'suggested',
      runbookId: runbook.id,
      message: `Queued for human approval: ${approvalId}`,
    };
  }
  ...
  if (activeRemediations.has(incident.component)) {
    ...
    console.log(
      `[blast-radius] Remediation already in progress for ${incident.component} (correlation=${active.correlationId}, runbook=${active.runbookId})`
    );
    const entry = {
      timestamp: new Date().toISOString(),
      agent: 'operator',
    ...
    return {
      action: 'logged',
      runbookId: runbook.id,
      message: `Remediation already in progress for ${incident.component}`,
    };
  }
  ...
  if (activeRemediations.size >= MAX_CONCURRENT_REMEDIATIONS) {
    console.log(
      `[blast-radius] Max concurrent remediations reached (${MAX_CONCURRENT_REMEDIATIONS}), skipping ${incident.component}`
    );
    const entry = {
      timestamp: new Date().toISOString(),
      agent: 'operator',
    ...
    return {
      action: 'logged',
      runbookId: runbook.id,
      message: `Max concurrent remediations (${MAX_CONCURRENT_REMEDIATIONS}) reached`,
    };
  }

  // Pre-dispatch verification: confirm incident is still active
  if (
    config.verifyBeforeDispatch &&
    ...
    if (!verification.confirmed) {
      ...
      persistAuditEntry(entry);
      console.log(
        `[verify] Incident on ${incident.component} no longer active: ${verification.reason}`
      );
      triageLatency.observe((Date.now() - startTime) / 1000);
      return {
        action: 'stale',
        runbookId: runbook.id,
        message: `Incident no longer active: ${verification.reason}`,
      };
    }
  ...
  activeRemediations.set(incident.component, {
    correlationId,
    runbookId: runbook.id,
  ...
  return {
    ...
    message: result.success
      ? `Dispatched runbook ${runbook.id} (attempt ${attempt})`
      : result.message,
  };
}
  ...
  if (body.from === 'mitigator' && body.type === 'audit') {
    ...
    for (const [component, active] of activeRemediations) {
      if (active.runbookId === runbookId) {
        ...
        console.log(
          `[blast-radius] Cleared active remediation for ${component} (runbook=${runbookId})`
        );
        break;
      }
    }
...
async function handleApproval(
  ...
  if (approve) {
    incidentsDispatched.inc();
    const result = await dispatchToMitigator(
      pending.incident,
      pending.runbookId,
...
const start = async () => {
  ...
  console.log(
    `Monitor: ${config.services.monitorUrl} | Mitigator: ${config.services.mitigatorUrl}`
  );
  console.log(
