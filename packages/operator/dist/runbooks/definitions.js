...
export function matchRunbook(component, evidence, severity) {
    const evidenceText = evidence.join(" ");
    for (const rb of runbooks) {
        const { severity: allowedSeverities, componentPattern, evidencePattern } = rb.incidentPattern;
        if (allowedSeverities && severity && !allowedSeverities.includes(severity))
            continue;
        if (componentPattern && !componentPattern.test(component))
            continue;
        if (evidencePattern && !evidencePattern.test(evidenceText))
            continue;
        return rb;
    }
    return undefined;
