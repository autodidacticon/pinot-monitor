...
async function forwardToOperator(incidents) {
...
async function handleSweepInner(_req, res, _signal) {
    sweepCount.inc();
    ...
    try {
        ...
        if (incidents.length > 0) {
            forwardToOperator(incidents).catch((err) => console.error(`Failed to forward incidents to operator: ${err instanceof Error ? err.message : err}`));
        }
...
async function handleChatInner(req, res, _signal) {
    let body;
    try {
