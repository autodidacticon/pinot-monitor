...
const sweepStore: SweepRecord[] = [];

/** Record a sweep in-memory and emit a structured JSON log to stdout. */
export function recordSweep(record: SweepRecord): void {
  // In-memory
...
export function getSweepHistory(lastHours?: number): SweepRecord[] {
  if (!lastHours) return [...sweepStore];
