...
export function storeIncidents(incidents: Incident[]): void {
...
export function getIncidents(severity?: Severity): Incident[] {
  if (severity) {
...
export function parseIncidents(response: string): Incident[] {
...
function extractFromReport(response: string): Incident[] {
  ...
  return incidents.filter((i): i is Incident => validateIncident(i) !== null);
}
