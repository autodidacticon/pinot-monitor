...
export const pinotTables = defineTool(
  ...
  z.object({
    tableName: z
      .string()
      .optional()
      .describe('If provided, get config for this table. Otherwise list all tables.'),
  }),
  async ({ tableName }) => {
...
export const pinotTableSize = defineTool(
  ...
  z.object({
    tableName: z
      .string()
      .optional()
      .describe('If provided, get size for this table. Otherwise get sizes for all tables.'),
  }),
  async ({ tableName }) => {
    if (tableName) {
      ...
      try {
        ...
        if (
          data.reportedSizePerReplicaInBytes &&
          typeof data.reportedSizePerReplicaInBytes === 'object'
        ) {
          result += `\nPer-replica sizes:\n`;
...
export const pinotBrokerLatency = defineTool(
  ...
  z.object({
    tableName: z
      .string()
      .optional()
      .describe('If provided, probe this table only. Otherwise probe all tables.'),
  }),
  async ({ tableName }) => {
    // Helper: probe a single table and measure latency
    async function probeTable(
      table: string
    ): Promise<{ table: string; latencyMs: number; status: string; error?: string }> {
...
export const pinotIngestionStatus = defineTool(
  ...
  z.object({
    tableName: z
      .string()
      .optional()
      .describe('If provided, check this specific table. Otherwise check all REALTIME tables.'),
  }),
  async ({ tableName }) => {
    ...
    async function checkTable(table: string): Promise<string> {
      ...
      try {
        ...
        for (const [segName, segInfo] of segmentEntries) {
          ...
          const partitionOffsetInfo = info.partitionOffsetInfo as
          ...
          if (partitionOffsetInfo) {
            const currentOffsets = partitionOffsetInfo.currentOffsetsMap as
              | Record<string, string>
              | undefined;
            const latestOffsets = partitionOffsetInfo.latestUpstreamOffsetMap as
              | Record<string, string>
              | undefined;
        ...
        if (
          raw.includes('does not have tableType REALTIME') ||
    ...
    const lines: string[] = [
      `Ingestion status report (checking ${tableNames.length} table(s) for REALTIME consumers):\n`,
    ];
    let realtimeCount = 0;
...
export const pinotServerMetrics = defineTool(
  ...
  async ({ component }) => {
    ...
    const components =
      component === 'all' ? (['controller', 'broker', 'server'] as const) : ([component] as const);
        ...
        try {
          ...
          for (const tbl of tableNames) {
            ...
            try {
              ...
              const segmentMap: Record<string, Record<string, string>> = extView.OFFLINE ??
              extView.REALTIME ??
              {};
