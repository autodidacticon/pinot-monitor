...
export const kubectlEvents = defineTool(
  'kubectl_events',
  'Get recent warning/error Kubernetes events (OOMKills, evictions, scheduling failures, image pull errors, etc.) in a namespace.',
  z.object({
    ...
    sinceMinutes: z
      ...
      .optional()
      .default(30)
      .describe('Only show events newer than N minutes (default 30)'),
  }),
  async ({ namespace, sinceMinutes }) => {
    const cmdArgs = [
      ...
      '-o',
      'custom-columns=TIMESTAMP:.lastTimestamp,TYPE:.type,REASON:.reason,OBJECT:.involvedObject.kind/.involvedObject.name,MESSAGE:.message',
      '--no-headers',
    ];
...
export const kubectlGet = defineTool(
  ...
  z.object({
    ...
    args: z
      .array(z.string())
      .describe('Additional arguments (resource type, name, flags like -o json)'),
  }),
