...
export const kubectlDelete = defineTool(
  'kubectl_delete',
  'Delete a Kubernetes resource (typically a pod to force restart via controller). Logs before/after state.',
  z.object({
    ...
    selector: z
      .string()
      .optional()
      .describe('Label selector instead of name (e.g., component=broker)'),
  }),
...
export const kubectlExec = defineTool(
  'kubectl_exec',
  'Execute a command inside a running pod. Use for diagnostics or config reloads.',
  z.object({
...
export const kubectlGet = defineTool(
  'kubectl_get_mitigator',
  'Read-only kubectl for the mitigator to capture state before/after mutations.',
  z.object({
